function getAllEmilyPacGyreGPS(timewindow,timenow,varargin)
...
% acquiring data
for i = 1:numel(EmilyPacificGyreInstruments.IMEI)
    try
        EmilyPacificGyreGPS(i) = acEmilyPacGyreGPS(EmilyPacificGyreInstruments.IMEI{i},...
...
try
    save('BuoyData/EmilyPacificGyreGPS.mat','EmilyPacificGyreGPS');
catch err
    fprintf(1,'%s: An error occured in getAllEmilyPacGyreGPS\n during saving GPS data\n',datestr(now));
    disp(err)
end
