function [ Data, Config, T_beam2xyz ] = signatureAD2CP_beam2xyz_enu_interp_prh( Data, Config, mode, twoZs )
...
T_beam2xyz = [    0.6782         0   -0.6782         0;...
    -0.2644    -0.3546    -0.2644    -0.3546;...
        0   1.1831         0    -1.1831]; %here, i've switched Y and Z from the normal martix because in the WW Y is pointing upwards
                                          % normal Y axis is the new Z
                                          % normal -Z axis is the Y
else
    if numberOfBeams == 3,
        T_beam2xyz = [beam2xyz_vec(1:3); beam2xyz_vec(4:6); beam2xyz_vec(7:9)];      % this will need to be corrected for WW if we want to do three beam solution ajl 10/08/15  
    end
end
...
% heading pitch and roll are not sample at the same pace than the
% velocities. we interpolate these data to smooth the results.
tt=Data.([dataModeWord  '_MatlabTimeStamp']);
dt=round(1/nanmean(diff(tt))/86400);
...
close
hold on
plot(tt(2000:2100),roll(2000:2100))
plot(tt(2000:2100),rolli(2000:2100),'r+-')
datetick('x')
...
for sampleIndex = 1:length(Data.( [dataModeWord  '_Error' ]));
   ...
   hh = pi*(headingi(sampleIndex)-90)/180;
   pp = pi*pitchi(sampleIndex)/180;
   rr = pi*rolli(sampleIndex)/180;
