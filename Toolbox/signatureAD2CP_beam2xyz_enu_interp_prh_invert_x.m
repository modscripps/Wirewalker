function [ Data, Config, T_beam2xyz ] = signatureAD2CP_beam2xyz_enu_interp_prh_invert_x( Data, Config, mode, twoZs )
...
% 	zAllCells( :, nCell ) = xyz( 3, : )';
	xAllCells( :, nCell ) = xyz( 1, : )';	
	yAllCells( :, nCell ) = xyz( 3, : )';
	zAllCells( :, nCell ) = -xyz( 2, : )';
	if twoZs == 1
...
for sampleIndex = 1:length(Data.( [dataModeWord  '_Error' ]));
   ...
   hh = pi*(headingi(sampleIndex))/180;
