function [std_profiles]=make_standard_profiles_AQDII_filt_surf(in,indx,P_vec)
...
for ii=1:length(indx)
    ...
    etemp=in.e_aqd_filt(indx(ii,1):indx(ii,2));
    ntemp=in.n_aqd_filt(indx(ii,1):indx(ii,2));
    utemp=in.u_aqd_filt(indx(ii,1):indx(ii,2));
    etemp=etemp(id);
    ntemp=ntemp(id);
    utemp=utemp(id);
